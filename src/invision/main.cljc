(ns invision.main
  "Kotodama WASM entrypoint for the Invision clean-room actor (L4) — Clojure port.
  ...
  facts (`invision.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "invision")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"invision.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "invision-compat" :tier tier :entities entities} 200])
...
(defn start! [] :invision-compat/ready)
